  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        ...
        // Creates a fresh peer, adds local tracks, but does NOT auto-send offer
        const createPeer = (remoteId: string): RTCPeerConnection => {
          ...
          // Always add audio + video from camera stream
          ...
          // If screen sharing, replace the video sender's track with the screen track
        ...
        // Send an offer to a specific remote user
        const sendOffer = async (remoteId: string) => {
          ...
          try {
            ...
            channel.send({
              type: 'broadcast',
              event: 'video-offer',
              payload: { sender: userId, target: remoteId, sdp: pc.localDescription },
            });
          } catch (err) {
        ...
        channel
          .on('broadcast', { event: 'video-offer' }, async ({ payload }) => {
            ...
            const remoteId = payload.sender;

            // If we're already making an offer to this peer, use "polite" peer logic
            // The peer with the lower ID is "polite" and rolls back
            const offerCollision = makingOffer.current.has(remoteId);
            const isPolite = userId < remoteId;

            if (offerCollision && !isPolite) {
              // We are impolite and already making an offer, ignore theirs
              return;
            }
          ...
          .on('broadcast', { event: 'video-join' }, async ({ payload }) => {
            ...
            // A new user joined — create a peer and send them an offer
            createPeer(payload.userId);
          ...
          .subscribe((st) => {
            if (st === 'SUBSCRIBED') {
              // Announce ourselves — existing peers will send us offers
                ...
                >
                  {u.stream ? (
                    <video
                      ...
                      muted={u.isLocal}
                      ref={(el) => {
                        if (el && u.stream && el.srcObject !== u.stream) el.srcObject = u.stream;
                      }}
